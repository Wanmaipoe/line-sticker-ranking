...
function RankBadge({ rank }: { rank: number }) {
  ...
  return <span className="flex-shrink-0 w-6 text-center text-sm font-bold text-gray-300 dark:text-gray-600 tabular-nums">{rank}</span>;
}
...
function DeltaChip({ delta, isNew }: { delta: number | null; isNew: boolean }) {
  if (isNew) {
    return (
      <span className="flex-shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded bg-sky-50 dark:bg-sky-500/15 text-sky-600 dark:text-sky-400">
  ...
  if (delta > 0) {
    return (
      <span className="flex-shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-500/15 text-green-600 dark:text-green-400 tabular-nums">
  ...
  if (delta < 0) {
    return (
      <span className="flex-shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-500/15 text-red-500 dark:text-red-400 tabular-nums">
  ...
  return (
    <span className="flex-shrink-0 text-[11px] text-gray-300 dark:text-gray-600 px-1.5">
...
function Thumb({ id, name, image_url, size = 48 }: { id: string; name: string; image_url: string | null; size?: number }) {
  return (
    <div className="rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-800 flex-shrink-0" style={{ width: size, height: size }}>
      <Image
...
export default function HomeClient({ initialDashboard, initialTrending }: HomeClientProps) {
  ...
  const revenueLink = (
    <a
      ...
      className="flex items-center gap-1.5 px-3 py-2.5 -mb-px text-sm font-medium border-b-2 border-transparent text-gray-400 dark:text-gray-500 hover:border-gray-300 dark:hover:border-gray-600 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
    >
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      {/* Top bar */}
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10 shadow-sm dark:ring-1 dark:ring-white/10">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
          ...
          <div className="relative flex-1 sm:max-w-lg">
            <div className="flex items-center gap-2">
              <div className="flex rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden text-xs flex-shrink-0">
                <button
                  onClick={() => handleModeChange('sticker')}
                  className={`px-2.5 py-2 transition-colors ${
                    searchMode === 'sticker' ? 'bg-[#06c755] text-white' : 'bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'
                  }`}
                >
                  Sticker
                </button>
                <button
                  ...
                  className={`px-2.5 py-2 transition-colors ${
                    searchMode === 'creator' ? 'bg-[#06c755] text-white' : 'bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'
                  }`}
                >
                  Creator
                </button>
              ...
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full px-4 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 focus:border-[#06c755] focus:outline-none text-sm transition-colors bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                />
              </div>
              <button
            ...
            {showDropdown && searchMode === 'sticker' && results.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-lg dark:ring-1 dark:ring-white/10 overflow-hidden z-20">
                {results.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => {
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-green-50 dark:hover:bg-green-500/10 transition-colors border-b border-gray-50 dark:border-gray-800 last:border-0 text-left"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500">ID: {p.id}</p>
                    </div>
                    <span className="text-xs text-green-500 dark:text-green-400 flex-shrink-0">View →</span>
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-lg dark:ring-1 dark:ring-white/10 overflow-hidden z-20">
                {creatorResults.map((c) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 px-4 py-2.5 hover:bg-green-50 dark:hover:bg-green-500/10 transition-colors border-b border-gray-50 dark:border-gray-800 last:border-0 text-left"
                  >
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-200">👤 {c.author}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">{c.count} packs</span>
                  </button>
                ))}
              </div>
            )}
            ...
            {showDropdown && !searching &&
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-lg dark:ring-1 dark:ring-white/10 p-4 text-center text-sm text-gray-400 dark:text-gray-500 z-20">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-10">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm dark:ring-1 dark:ring-white/10 px-5 py-5 sm:px-7 sm:py-6">
          <div className="mb-3">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-300">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            ...
            <a
              href="/favorites"
              className="text-sm font-medium bg-white dark:bg-gray-900 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-500/30 px-4 py-2 rounded-xl hover:bg-green-50 dark:hover:bg-green-500/10 transition-colors"
            >
              Track your pack
            </a>
            <span className="text-xs text-gray-400 dark:text-gray-500 sm:ml-auto w-full sm:w-auto">
        ...
        <section id="top5" className="scroll-mt-20">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-700 dark:text-gray-200 text-base">🏆 Top 5 per country</h2>
            <span className="text-xs text-gray-400 dark:text-gray-500 hidden sm:inline">Tap a sticker for its 30-day history</span>
          </div>

            {loadingDash && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl p-4 animate-pulse h-72" />
                ))}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {dashboard?.countries.filter((c) => HOME_COUNTRIES.includes(c.code)).map((country) => (
                <div
                  key={country.code}
                  className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:ring-1 dark:ring-white/10 border border-gray-100 dark:border-gray-800 overflow-hidden flex flex-col"
                >
                  <div className="px-4 py-3 border-b border-gray-50 dark:border-gray-800 flex items-center gap-2">
                    <span className="text-xl">{country.flag}</span>
                    <span className="font-semibold text-sm text-gray-700 dark:text-gray-200">{country.name}</span>
                    <span className="ml-auto text-[11px] uppercase tracking-wide text-gray-300 dark:text-gray-600 font-semibold">Top 5</span>
                  </div>
                    ...
                    {country.top5.map((item) => (
                      ...
                      >
                        <Link href={`/sticker/${item.id}`} className="flex items-center gap-2.5 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm text-gray-700 dark:text-gray-200 font-medium truncate group-hover:text-green-700 dark:group-hover:text-green-300 leading-tight">
                              {item.name}
                            </div>
                            {item.spark.length >= 2 && (
                              <div className="mt-1 flex items-center gap-1.5">
                                <Sparkline ranks={item.spark} />
                                <span className="text-[11px] text-gray-400 dark:text-gray-500 flex-shrink-0 leading-none">past 7 days</span>
                              </div>
                            )}
                          </div>
                        ...
                        >
                          <span className={isFavorite(item.id) ? 'text-red-400 dark:text-red-400' : 'text-gray-300 dark:text-gray-600 hover:text-red-400 dark:hover:text-red-400'}>
                            {isFavorite(item.id) ? '♥' : '♡'}
                          </span>
                        </button>
                      </div>
                  ...
                  <div className="px-3 py-2.5 border-t border-gray-50 dark:border-gray-800">
                    <a
                      href={`/country/${country.code}`}
                      className="text-xs font-medium text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10 hover:bg-green-100 dark:hover:bg-green-500/20 rounded-lg py-2 w-full text-center block transition-colors"
                    >
                      View full ranking →
                    </a>
                  </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-700 dark:text-gray-200 text-base">🔥 Movers — biggest jumps today</h2>
            {(() => {
          ...
          {loadingTrend && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl p-4 animate-pulse h-52" />
              ))}
            </div>
          )}

          {!loadingTrend && !trending?.countries?.length && (
            <div className="text-center py-10 text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800">
              <p className="text-3xl mb-2">⏱️</p>
              <p className="text-sm">Today&apos;s movers appear as the day&apos;s hourly snapshots build up.</p>
              <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">Rankings refresh every hour around :30 (BKK).</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {trending?.countries.filter((c) => HOME_COUNTRIES.includes(c.code)).map((country) => (
              <div
                key={country.code}
                className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:ring-1 dark:ring-white/10 border border-gray-100 dark:border-gray-800 overflow-hidden"
              >
                <div className="px-4 py-3 border-b border-gray-50 dark:border-gray-800 flex items-center gap-2">
                  <span className="text-xl">{country.flag}</span>
                  <span className="font-semibold text-sm text-gray-700 dark:text-gray-200">{country.name}</span>
                ...
                <div className="p-2">
                  {country.trending.map((item) => (
                    ...
                    >
                      <span className="flex-shrink-0 w-10 text-center text-[11px] font-semibold px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-500/15 text-green-600 dark:text-green-400 tabular-nums">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-gray-400 dark:text-gray-500 leading-tight tabular-nums">
                          now #{item.current_rank} · was #{item.old_rank}
                        </div>
                      </div>
                      {item.improvement >= HOT_JUMP && (
                        <span className="flex-shrink-0 text-[11px] font-semibold px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-500/15 text-red-500 dark:text-red-400">
        ...
        <section aria-labelledby="faq-heading" className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 dark:ring-1 dark:ring-white/10 p-6">
          <h2 id="faq-heading" className="font-bold text-gray-800 dark:text-gray-100 text-lg mb-4">
            LINE sticker rankings — questions &amp; answers
          </h2>
          {updatedAbs && (
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">Rankings last updated {updatedAbs}.</p>
          )}
          <div className="space-y-4">
            {faqs.map((f) => (
              <div key={f.q}>
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200">{f.q}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 leading-relaxed">{f.a}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {showCountryPicker && (
        ...
        >
          <div className="absolute inset-0 bg-black/40 dark:bg-black/60" onClick={() => setShowCountryPicker(false)} />
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-xl dark:ring-1 dark:ring-white/10 border border-gray-100 dark:border-gray-800 w-full max-w-sm p-6">
            <button
              onClick={() => setShowCountryPicker(false)}
              aria-label="Close"
              className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 text-xl leading-none transition-colors"
            >
              ×
            </button>
            <h2 className="font-bold text-gray-800 dark:text-gray-100 text-lg">Explore rankings</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">Pick a country to see its full Top 50.</p>
            <div className="space-y-2">
              {HOME_COUNTRY_CHIPS.map((c) => (
                <a
                  key={c.code}
                  href={`/country/${c.code}`}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-green-300 dark:hover:border-green-500/40 hover:bg-green-50 dark:hover:bg-green-500/10 transition-colors"
                >
                  <span className="text-2xl" aria-hidden>{c.flag}</span>
                  <span className="font-medium text-gray-700 dark:text-gray-200">{c.name}</span>
                  <span className="ml-auto text-xs text-gray-400 dark:text-gray-500">Top 50 →</span>
                </a>
              ))}
            </div>
          </div>
